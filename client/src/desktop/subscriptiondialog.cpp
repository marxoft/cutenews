...
void SubscriptionDialog::onSubscriptionFetched(DBConnection *connection) {
    if (connection->status() == DBConnection::Ready) {
        ...
        if (!result.isEmpty()) {
            m_sourceEdit->setText(result.value("source").toString());
            m_sourceButton->setEnabled(result.value("sourceType", Subscription::Url) != Subscription::Url);
            m_enclosuresCheckBox->setChecked(result.value("downloadEnclosures", false).toBool());
