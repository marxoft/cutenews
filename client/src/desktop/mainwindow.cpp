    ...
    m_transfersPage(0)
{
    ...
    connect(DBNotify::instance(), SIGNAL(error(QString)), this, SLOT(onError(QString)));
    ...
    connect(ServerSettings::instance(), SIGNAL(error(QString)), this, SLOT(onError(QString)));
...
void MainWindow::setCurrentArticle(const QModelIndex &index) {
    ...
    m_browser->setHtml(index.data(Article::BodyRole).toString(), Settings::serverAddress());
