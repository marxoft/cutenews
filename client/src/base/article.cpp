    ...
    if (enabled != autoUpdate()) {
        ...
        if (enabled) {
            ...
            connect(DBNotify::instance(), SIGNAL(articleRead(QString, QString, bool, QVariantMap)),
                    this, SLOT(onArticleRead(QString, QString, bool, QVariantMap)));
...
void Article::load(const QVariantMap &properties) {
    setAuthor(properties.value("author").toString());
    setBody(properties.value("body").toString());
    setCategories(properties.value("categories").toStringList());
    setDate(QDateTime::fromTime_t(properties.value("date").toUInt()));
    setEnclosures(properties.value("enclosures").toList());
    setFavourite(properties.value("favourite").toBool());
    setRead(properties.value("read").toBool());
...
void Article::onArticleFavourited(const QString &articleId, bool, const QVariantMap &properties) {
    if (articleId == id()) {
        load(properties);
    }
}

void Article::onArticleRead(const QString &articleId, const QString &, bool, const QVariantMap &properties) {
    if (articleId == id()) {
        load(properties);
    }
}
