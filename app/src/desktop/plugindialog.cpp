    ...
    m_subscriptionId(subscriptionId)
{
    ...
    DBConnection::connection(this, SLOT(onSubscriptionFetched(DBConnection*)))->exec(QString("SELECT source, downloadEnclosures, updateInterval FROM subscriptions WHERE id = '%1'").arg(subscriptionId));
}
...
void PluginDialog::onSubscriptionFetched(DBConnection *connection) {
    if (connection->status() == DBConnection::Ready) {
        if (connection->nextRecord()) {
            m_enclosuresCheckBox->setChecked(connection->value(1).toBool());
            const QVariantMap source = QtJson::Json::parse(connection->value(0).toString()).toMap();
